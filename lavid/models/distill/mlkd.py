...
class MLKD(Distiller):
    def __init__(self, student, teacher, cfg):
        ...
        self.kd_loss_weight = cfg.ALPHA
        self.logit_stand = cfg.LOGIT_STAND 

    def forward_train(self, image_weak, image_strong, target, **kwargs):
        _, logits_student_weak = self.student(image_weak, is_feat=True)
        _, logits_student_strong = self.student(image_strong, is_feat=True)
        with torch.no_grad():
            _, logits_teacher_weak = self.teacher(image_weak, is_feat=True)
            _, logits_teacher_strong = self.teacher(image_strong, is_feat=True)
